...
for V=Vmin:Vstep:Vmax
    ...
    [x,fval,exitflag,output] = fsolve(@Model_Function_Chio_1_Final,x0,options) ;
...
%pH
Vvector = linspace(1e-5,2.00001,20001) ;
...
figure(4)
plot(pHsave,(10.^(-logSQsave)+10.^(-logHSQsave))./(10.^(-logH2Qsave) + 10.^(-logHQsave) + 10.^(-logQ2save) + 10.^(-logQsave)  ))
...
%datamatrix = [flip(pHsave') flip(SQtotal') flip(logSQsave') flip(logHSQsave') flip(logH2Qsave') flip(logHQsave') flip(logQ2save') flip(logQsave')   flip(Esave')] ;

datamatrix = [(pHsave') (SQtotal') (logSQsave') (logHSQsave') (logH2Qsave') (logHQsave') (logQ2save') (logQsave')  (Esave')] ;